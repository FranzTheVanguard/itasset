                ...
                <div class="card border-0 shadow rounded">
                    <div class="card-body">
                      <div class="d-flex flex-row-reverse">
                        <a href="{{ route('peminjamans.create') }}" class="btn btn-outline-secondary mb-3"><i class="fas fa-plus"></i>&nbsp;&nbsp;Add</a>
                      </div>
                            ...
                            <tbody>
                              @forelse ($peminjamans as $peminjaman)
                                <tr>
                                  ...
                                  <td class="text-center">{{ \Carbon\Carbon::createFromTimeStamp(strtotime($peminjaman->tanggal_pinjam))->format('d/m/Y')}}</td>
                                  <td class="text-center">{!! $peminjaman->keterangan_peminjaman !!}</td>
                                  <td class="text-center">{!! $peminjaman->stock->nama_komputer !!}</td>
                                  <td class="text-center">{!! $peminjaman->stock->serial_number !!}</td>
                                  <td class="text-center">{!! $peminjaman->status !!}</td>
                                    <td class="text-center">
                                    <form onsubmit="return confirm('Apakah Anda Yakin ?');" action="{{ route('peminjamans.destroy', $peminjaman->id) }}" method="POST">
                                      <div class="d-flex my-auto">
                                        <div class="mr-2">
                                            <a href="{{ route('peminjamans.edit', $peminjaman->id) }}" class="btn btn-sm btn-primary"><i class="fas fa-pen"></i></a>
                                          </div>
                                          <div>
                                            @csrf
                                            @method('DELETE')
